...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['excluir_id'])) {
    ...
    $stmt = $conexao->delete("DELETE FROM trens WHERE id_trem = ?");
        ...
        <tbody>
            <?php while($linha = $resultado->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><?= number_format($linha['capacidade_toneladas'], 2, ',', '.') ?></td>
                    <td>
                        <span class="etiqueta etiqueta-<?= strtolower(htmlspecialchars($linha['situacao_trem'])) ?>">
